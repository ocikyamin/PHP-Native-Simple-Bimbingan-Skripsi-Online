...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    if ($pesan == "") {
        ...
        if ($file_name == "") {
            $query = mysqli_query($con, "INSERT INTO `tb_forum`(`kode`,`user_id`,`user_id_to`, `user_type`, `pesan`,`reply_to`,`document`) VALUES ('$kode','$userId','$to_user','dsn','$pesan','$pesan_id','nofile')");
            if ($query) {
            ...
            if (in_array($file_ext, $array) === true) {
                if ($file_size < 2000000) {
                    ...
                    $query = mysqli_query($con, "INSERT INTO `tb_forum`(`kode`,`user_id` ,`user_id_to`, `user_type`, `pesan`,`reply_to`,`document`) VALUES ('$kode','$userId','$to_user','dsn','$pesan','$pesan_id','$dok')");
                    if ($query) {
                        mysqli_query($con, "UPDATE tb_forum SET pesan_status='Y' WHERE id=$pesan_id ");
